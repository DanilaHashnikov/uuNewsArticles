...
const Errors = require("../api/errors/articles-main-error.js");
...
const logger = LoggerFactory.get("ArticlesMainAbl");

class ArticlesMainAbl {
  constructor() {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["articlesMain", "newspaper", "topic", "author", "article"];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "NewsArticles",
        typeCode: "news-articles-maing01",
        location: location,
    ...
    try {
      uuTodoInstance = await this.mainDao.create(uuObject);
    } catch (e) {
      throw new Errors.Init.articlesMainCreateDaoFailed({ uuAppErrorMap }, e);
    }
...
module.exports = new ArticlesMainAbl();
